/*
    This code connects the Arduino to wifi and sends the measured 
    to and mqtt server. Code was written to support Lambda sensor
    in wood stoves. 

    Author: https://github.com/zegerk
    
    Code is based on the example from Bylund Automative AB:
    http://www.bylund-automotive.com/ or info@bylund-automotive.com
    ...
    2022-12-01        v1.0.0        First release to GitHub.
...
const char broker[] = "xxx.xxx.xxx.xxx"; // IP adress of the mqtt broker
int        port       = 1883; // port of the mqtt broker

/**
 * mqtt topics where data will be send
 */
const char topic[]          = "lambda/status";
const char topicError[]     = "lambda/error";
const char topicHeartbeat[] = "lambda/heartbeat";
const char topicOxygen[]    = "lambda/oxygen";
const char topicLambda[]    = "lambda/lambda";
const char topicUA[]        = "lambda/ua";
const char topicUR[]        = "lambda/ur";
const char topicUB[]        = "lambda/ub";
...
bool ledStatus = false;   // Blinking the led when message is send
...
void connectWifi() {  
  ...
  while (status != WL_CONNECTED) {
    ...
    /**
     * Take some extra time for connecting
     */
    ...
    // WL_DISCONNECTED    = 6
    while (loop_counter++ < 10 && status != WL_CONNECTED) {
      delay(1000);

      /**
       * Wifi.begin is looping 
       * (https://github.com/arduino-libraries/WiFiNINA/blob/094856b05a80ea2f79cf956c6ac5ff6715546db5/src/WiFi.cpp#L148)
       */
    ...
    /**
     * Just reset the whole device if no connection can be made
     */
    if (loop_counter == 11) {
      Serial.println("Wifi failed, resetting");
      asm volatile (" jmp 0");  //call reset
    }
  }  
...
void start() {
  ...
  while (adcValue_UB < UBAT_MIN || CJ125_Status != CJ125_DIAG_REG_STATUS_OK) {
    ...
    if (CJ125_Status != CJ125_DIAG_REG_STATUS_OK) {
      ...
      mqttClient.beginMessage(topicError);
      mqttClient.print(CJ125_Status, HEX);
...
void loop() {
  ...
  //Display on serial port and send to mqtt at defined rate.
  //Comma separate values, readable by frontends.
  if ( (100 / SERIAL_RATE) ==  serial_counter) {
    ...
    if (CJ125_Status == CJ125_DIAG_REG_STATUS_OK) {
      ...
      mqttClient.beginMessage(topicUA);
      mqttClient.print(String(adcValue_UA, DEC));
      mqttClient.endMessage();  

      txString += ", UR_ADC: ";
      txString += String(adcValue_UR, DEC);
      mqttClient.beginMessage(topicUR);
      mqttClient.print(String(adcValue_UR, DEC));
      mqttClient.endMessage();        

      txString += ", UB_ADC: ";
      txString += String(adcValue_UB, DEC);
      mqttClient.beginMessage(topicUB);
      mqttClient.print(String(adcValue_UB, DEC));
      mqttClient.endMessage();      
      ...
      if (adcValue_UA >= 39 && adcValue_UA <= 791) {
          ...
          mqttClient.beginMessage(topicLambda);
          mqttClient.print(String(LAMBDA_VALUE, 2));
          mqttClient.endMessage();
      } else {
          txString += ", Lambda: -";
      }
      ...
      if (adcValue_UA >= 307) {
        ...
        mqttClient.beginMessage(topicOxygen);
        mqttClient.print(String(OXYGEN_CONTENT, 2));
        mqttClient.endMessage();
